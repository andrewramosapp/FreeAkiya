...
type Subscriber = { id?: string; email: string; name: string; since: string; tier: string; beehiivId?: string; stripeSubId?: string; openRate?: string; emailsSent?: number | string; emailsOpened?: number | string; };
type EmailMsg = { id: string; direction: string; from_addr: string; to_addr: string; subject: string; body_text: string; created_at: string; status: string; thread_id: string; };

const TABS = ["📊 Stats", "📬 Inquiries", "🤝 Referrals", "📧 Email", "🏯 Listings", "🔄 Scraper", "👥 Subscribers", "📤 Newsletter"] as const;
...
export default function AdminPage() {
  ...
  const load = useCallback(async (tab: Tab) => {
    if (tab === "📊 Stats") {
      ...
      const r = await fetch("/api/admin/referrals", { headers: h });
      if (r.ok) setReferrals((await r.json()).referrals || []);
    } else if (tab === "📬 Inquiries") {
      const r = await fetch("/api/inquiries", { headers: h });
  ...
  useEffect(() => { if (authed) load(activeTab); }, [authed, activeTab]);
  ...
  }, [emailFolder, emailQ, emailPage, secret]);
  ...
  const sendCompose = async () => {
    ...
    const r = await fetch("/api/email", { method: "POST", headers: { ...h, "Content-Type": "application/json" },
      body: JSON.stringify({ to: composeTo, subject: composeSubject, body: composeBody, fromInbox: composeFrom,
        replyToId: selectedEmail?.id, replyToMessageId: selectedEmail?.thread_id }) });
    setComposeSending(false);
    if (r.ok) { setComposeSent(true); setComposing(false); loadEmails(0); setTimeout(()=>setComposeSent(false),3000); }
  };
        ...
        <div className="flex gap-1 px-6 max-w-6xl mx-auto">
          {TABS.map(tab => (
            <button key={tab} onClick={() => { setActiveTab(tab); load(tab); }}
              ...
              {tab}{tab === "📬 Inquiries" && inquiries.filter(i=>i.status==="new").length > 0 ? ` (${inquiries.filter(i=>i.status==="new").length})` : ""}
            </button>
          ))}
        </div>
      </div>
        ...
        {activeTab === "🤝 Referrals" && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-2xl font-black">Agent Referrals</h1>
              <button onClick={()=>load("🤝 Referrals")} className="text-xs text-gray-600 hover:text-white">↻</button>
            </div>
            {/* Export bar */}
        ...
        {activeTab === "📬 Inquiries" && (
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => load("📬 Inquiries")} className="text-xs text-gray-600 hover:text-white">↻ Refresh</button>
            </div>
            {inquiries.length === 0 ? (
              <div className="text-center py-20"><div className="text-4xl mb-3">📭</div><p className="text-gray-600">No inquiries yet.</p></div>
            ) : (
              <div className="space-y-3">
                {inquiries.map(inq => (
                  <div key={inq.id} className={`rounded-2xl border p-5 cursor-pointer ${inq.status==="new" ? "border-[#e85d2f]/40 bg-[#e85d2f]/5" : "border-white/10 bg-white/3"}`}
                    onClick={() => { setExpandedId(expandedId===inq.id?null:inq.id); if(inq.status==="new") markRead(inq.id); }}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          {inq.member_tier==="premium" && <span className="bg-[#e85d2f]/20 text-[#e85d2f] text-xs px-2 py-0.5 rounded-full">⭐ Premium</span>}
                        </div>
                        <div className="text-gray-500 text-xs mb-1">Re: <a href={inq.listing_url} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} className="hover:text-white">{inq.listing_name||inq.listing_slug}</a> {inq.listing_price && `· ${inq.listing_price}`}</div>
                        <p className={`text-gray-400 text-sm ${expandedId===inq.id?"":"truncate"}`}>{inq.message}</p>
                      </div>
                      <div className="text-gray-700 text-xs flex-shrink-0">{new Date(inq.created_at).toLocaleDateString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}</div>
                    </div>
                    {expandedId===inq.id && (
                      <div className="mt-4 pt-4 border-t border-white/10">
                        <p className="text-gray-300 text-sm whitespace-pre-wrap mb-4">{inq.message}</p>
                        <div className="flex gap-3 flex-wrap">
                          <button onClick={e=>{e.stopPropagation();setActiveTab("📧 Email");setComposing(true);setComposeTo(inq.email);setComposeSubject(`Re: Your inquiry about ${inq.listing_name||inq.listing_slug}`);setComposeBody(`Hi ${inq.name},

`);}} className="bg-[#e85d2f] hover:bg-[#d44f23] text-white text-sm font-bold px-4 py-2 rounded-full transition">✉ Reply in app →</button>
                        </div>
                      </div>
                    )}
                  </div>
